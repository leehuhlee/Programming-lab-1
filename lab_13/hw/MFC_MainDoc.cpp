...
BOOL CMFC_MainDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
	...
	// read data file
	char pathName[ 256 ];
	int pathSize = WideCharToMultiByte( CP_ACP, 0, lpszPathName, -1, pathName, 0, NULL, NULL );
	WideCharToMultiByte( CP_ACP, 0, lpszPathName, -1, pathName, pathSize, NULL, NULL );
	if( readFile( pathName ) )
	{
...
void CMFC_MainDoc::OnFileClose() 
{
	...
	// delete user data
	DeleteContents();
	SetPathName(L"C:\\", FALSE);
	AfxGetApp()->m_pMainWnd->SetWindowText(L"제목없음 - MFC_Main");
...
void CMFC_MainDoc::OnUpdateToolsDfs(CCmdUI *pCmdUI)
{
	...
	// if user data loaded, enable "DFS" menu
	pCmdUI->Enable(m_bDataLoaded);
}

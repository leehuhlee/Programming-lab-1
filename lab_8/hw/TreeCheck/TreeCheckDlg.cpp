...
BEGIN_MESSAGE_MAP(CTreeCheckDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON3, &CTreeCheckDlg::OnExit_)
END_MESSAGE_MAP()


// CTreeCheckDlg 메시지 처리기
...
{
	char *Result;
	if(currentCase<testCaseNum)
		currentCase++;
	Result=F3(currentCase);
	m_Result.SetWindowText(Result);
}

void CTreeCheckDlg::OnExit_()
{
	this->EndDialog(0);
}